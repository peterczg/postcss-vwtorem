...
const defaults = {
  // For instance，10vw will be converted to 1rem
  rootValue: 10,
  unitPrecision: 5,
  selectorBlackList: [],
  // By default, size over 100vw will not be converted
  // Because 100vw represents screen width which is considered not to be converted.
  rule: function (vwSize) { return vwSize < 100 },
  propList: ["*"],
  replace: true,
  mediaQuery: false,
  minPixelValue: 0,
...
module.exports = (options = {}) => {
  ...
  return {
    ...
    Declaration (decl) {
      ...
      // filter rule
      const sizeLike = parseFloat(decl.value)
      if (!Number.isNaN(sizeLike) && !opts.rule(sizeLike)) return
